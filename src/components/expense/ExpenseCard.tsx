...
export function ExpenseCard(props: Props) {
    ...
    return (
        <Card border="grey" onClick={props.ShowIdCard} className={`mt-3 w-100 ${!!props.expense.amount_reimbursement && parseFloat(props.expense.amount_reimbursement) < 1 ? 'border border-danger' : ''}`}>
            <Card.Header>
            ...
            <Card.Body>
                <Card.Title className='mb-3'>{props.expense.category.title}</Card.Title>
                    <div className="row mt-2">
                        <div className="col-8">
                            <p className='m-0 p-1'>Importo {props.expense.currency} {props.expense.amount_currency}</p>
