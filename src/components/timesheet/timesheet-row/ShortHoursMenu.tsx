...
import { calculateMaximumWorkingHours, calculateTaskHoursForDay } from "../utils/entryUtils";
...
export const ShortHoursMenu = React.memo<ShortHoursMenuProps>((props) => {
  ...
  const menuData = useMemo(() => {
    ...
    const allDates = getDateRange(startDate, endDate)
      .map(normalizeDate)
      .filter((date) => !dayEntryByDate.get(date)?.closed);
  ...
  }, [openShortMenu, day, taskId, taskEntries, dayEntries]);
  ...
  const submitHours = useCallback(
    async (value: number, selectedDates?: string[]) => {
      ...
      const invalidDate = datesToSave.find((date) => {
        const dayEntry = dayEntries.find(
        ...
        const otherTaskHours = calculateTaskHoursForDay(taskEntries, dayEntries, date, taskId);
        return otherTaskHours + value > maximumWorkingHours;
      });

      if (invalidDate) {
        toast.error(`Working hours exceed the available hours for ${invalidDate}.`);
        return;
      }
