...
export const TimeSheetRow: React.FC<TimeSheetRowProps> = ({
  ...
  const renderDayCell = (day: Date, dayIndex: number) => {
    ...
    const hasTaskEntriesForDay = Boolean(
      dayEntry && timesheet.taskEntries?.some((entry) => entry.dayEntry === dayEntry.id)
    );

    const type: TaskEntryCellType =
      dayEntry?.askedHoliday || (dayEntry?.isHoliday && !hasTaskEntriesForDay)
