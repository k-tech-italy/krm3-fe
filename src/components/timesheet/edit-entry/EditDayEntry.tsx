...
export default function EditDayEntry({
  ...
  useEffect(() => {
    ...
    for (const date of dates) {
      ...
      const scheduledHours = Number(
        schedule[normalizedDate] ?? schedule[normalizedDate.replaceAll("-", "_")] ?? 0
      );
      if (scheduledHours < totalHours) {
        setLeaveHoursError(
          `No overtime allowed when logging leave, special leave or rest hours. Maximum allowed for ${normalizeDate(date)}
            is ${scheduledHours} hours, Total hours: ` + totalHours
        );
        return;
      }
    }
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (isSpecialReasonMissing) return;

    const bankHoursNotEmpty = (bankFrom && bankFrom > 0) || (bankTo && bankTo > 0);
    const hasHours =
      (leaveHours && leaveHours > 0) ||
      (restHours && restHours > 0) ||
      (specialLeaveHours && specialLeaveHours > 0);
  ...
  const clearableDayEntries = dayEntries.filter((entry) => {
    ...
    return isDayInRange(fromDate, toDate, normalizedDay) && !entry.closed && !entry.isHoliday;
  });
              ...
              <div>
                ...
                {!!specialReasonOptions && (
                  <select
                    ...
                    className="w-full border border-app rounded-md p-2.75 "
                    disabled={readOnly}
                  >
