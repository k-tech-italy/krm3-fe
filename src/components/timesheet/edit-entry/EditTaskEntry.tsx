...
import { calculateMaximumWorkingHours, calculateTaskHoursForDay } from "../utils/entryUtils.ts";
...
export default function EditTaskEntry({
  ...
  const getDatesByEntryPresence = (rangeStart: Date, rangeEnd: Date) => {
    const allDates = getDatesBetween(rangeStart, rangeEnd, noWorkingDays, false);
  ...
  const workingHours = dayShiftHours + nightShiftHours + travelHours;
  const invalidWorkingHoursDate = getDatesToSave().find((date) => {
    const dayEntry = dayEntries.find((entry) => normalizeDate(entry.day) === normalizeDate(date));
    if (!dayEntry) return false;

    const maximumWorkingHours = calculateMaximumWorkingHours(dayEntry);
    if (maximumWorkingHours === null) return false;

    const otherTaskHours = calculateTaskHoursForDay(taskEntries, dayEntries, date, task.id);
    return otherTaskHours + workingHours > maximumWorkingHours;
  });

  const submit = (event: React.FormEvent) => {
    event.preventDefault();
    if (invalidWorkingHoursDate) return;
  ...
  return (
    <form onSubmit={submit} className="space-y-6" id="edit-task-entry-container">
      ...
      {invalidWorkingHoursDate && (
        <ErrorMessage
          message={`Working hours exceed the available hours for ${invalidWorkingHoursDate}.`}
        />
      )}
      ...
      >
        ...
        <div className="flex space-x-3">
          ...
          />
          <Krm3Button
            disabled={totalHours > 24 || totalHours === 0 || !!invalidWorkingHoursDate || readOnly}
            type="submit"
            style="primary"
            label="Save"
