...
export function useDragAndDrop({ scheduledDays, timesheet, callbacks }: UseDragAndDropProps) {
  ...
  const parseActiveId = (activeId: string) => {
    if (activeId.startsWith("column-")) {
      return { type: "column" as const, dayIndex: Number(activeId.replace("column-", "")) };
    }
    const parts = activeId.split("-");
    if (parts.length === 3) {
      return { date: parts[0], taskId: Number(parts[1]), entryId: Number(parts[2]) };
    }
    return null;
  };

  // Find a time entry by entryId and taskId
  const findTimeEntry = (entryId: number, taskId: number): TimeEntry | undefined =>
    timesheet?.timeEntries?.find((entry) => entry.id === entryId && entry.task === taskId);
  ...
  const handleColumnDragMove = (over: any) => {
    if (!over.id.startsWith("column-") || draggedColumnIndex.current === null) return;
  ...
  const handleCellDragMove = (over: any) => {
    ...
    if (!activeDragData.current?.taskId || Number(targetTaskId) !== activeDragData.current.taskId) return;
  ...
  const handleColumnDragEnd = (over: any) => {
    if (!over.id.startsWith("column-") || !activeDragData.current?.columnDay) return;
  ...
  const handleCellDragEnd = (over: any) => {
    ...
    if (!dragData || !dragData.taskId || Number(targetTaskId) !== dragData.taskId) return;
  ...
  const isColumnHighlighted = (dayIndex: number): boolean => {
    ...
    return dragType === "column" && draggedOverCells.some(day => scheduledDays[dayIndex].toDateString() === day.toDateString());
  };
